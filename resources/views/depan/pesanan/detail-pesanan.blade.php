...
@section('content')
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="card shadow-sm p-4">
                                ...
                                <td>
                                    @if ($pesanan->bukti_transfer)
                                        <img src="{{ asset('storage/' . $pesanan->bukti_transfer) }}" alt="Bukti Transfer"
                                            class="img-thumbnail" style="max-width: 150px; cursor: pointer;" data-bs-toggle="modal"
                    ...
                    <a href="https://wa.me/6282377885282?text={{ urlencode(
        'Halo Admin, saya ingin menanyakan detail pesanan saya:' . "\n" .
        'Nama: ' . ($pesanan->user->nama ?? '-') . "\n" .
        'Tanggal Pesanan: ' . $pesanan->created_at->format('d M Y, H:i') . "\n" .
        'Status: ' . ucfirst($pesanan->status) . "\n" .
        'Produk: ' . ($pesanan->produk->nama ?? 'Kaos Kustom') . "\n" .
        'Ukuran: ' . $pesanan->ukuran . "\n" .
        'Jumlah: ' . $pesanan->jumlah . "\n" .
        'Mohon informasinya, terima kasih.'
    ) }}" target="_blank" class="btn btn-success mt-2 w-100">
                        <i class="bi bi-whatsapp"></i> Hubungi Admin
                    </a>

                    <a href="{{ route('pesanan.user.index') }}" class="btn btn-secondary mt-3"><i
    ...
    @if ($pesanan->produk && $pesanan->produk->gambar)
        <div class="modal fade" id="modalGambarProduk" tabindex="-1" aria-labelledby="gambarProdukLabel" aria-hidden="true">
        ...
        <div class="modal fade" id="modalGambarProdukBelakang" tabindex="-1" aria-labelledby="gambarProdukBelakangLabel"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content bg-dark">
                    ...
                    <div class="modal-body text-center p-4">
                        <img src="{{ asset('storage/' . $pesanan->produk->gambar_belakang) }}" class="img-fluid rounded shadow"
                            style="max-height: 80vh;" alt="Gambar Produk Belakang">
                    </div>
                </div>
            </div>
        </div>
    ...
    @if ($pesanan->kaosKustom)
        <div class="modal fade" id="modalDesainDepan" tabindex="-1" aria-labelledby="modalDesainDepanLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content bg-dark">
                    <div class="modal-header border-0">
                    ...
                    <div class="modal-body text-center p-4">
                        <img src="{{ asset('storage/' . $pesanan->kaosKustom->desain_depan) }}" class="img-fluid rounded shadow"
                            style="max-height: 80vh;" alt="Desain Depan">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="modalBuktiTransfer" tabindex="-1" aria-labelledby="buktiTransferLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-dark">
                <div class="modal-header border-0">
